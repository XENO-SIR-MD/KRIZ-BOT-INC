...
const SparkyApp = require('./pluginsCore/app.js');
const sparky = new SparkyApp();
      ...
      if (args) {
          ...
          await sparky.update();
            const interval = setInterval(async () => {
                  const status = await sparky.deploymentInfo()
                  if(status === 'STARTING') {
...
Sparky({
	...
	async ({
		...
		const setVariable = await sparky.setVar(key.trim().toUpperCase(), value.trim());
...
Sparky({
	...
	async ({
		...
		const delVariable = await sparky.deleteVar(args.trim().toUpperCase());
...
Sparky({
	...
	async ({
		...
		const getVariables = await sparky.getVars();
...
async ( {
	...
	if (args?.toLowerCase() == "public" || args?.toLowerCase() == "private"){
		await sparky.setVar("WORK_TYPE",args,m)
...
Sparky({
    ...
    if (settingsContext.step === "menu" && m.quoted?.key?.id === settingsContext.quotedId) {
        ...
        await sparky.setVar(settingsContext.env_var, status === "on" ? "true" : "false");
...
async ({ m, args, client }) => {
    ...
    await m.reply(`_Added @${newSudo} as sudo_`, {
        mentions: [`${newSudo}@s.whatsapp.net`],
    });

    await sparky.setVar("SUDO", setSudo, m);
});
...
async ({ m, args, client }) => {
    ...
    await m.reply(`_Removed @${delSudo} from sudo_`, {
